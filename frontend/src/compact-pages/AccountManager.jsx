...
const AccountManager = () => {
    ...
    const Headers = [
        ...
        {label:"", val:(data)=>{
            return <div className="flex flex-row justify-end">
                <button onClick={()=>setSelectAccount(data?.id)}>
    ...
    useEffect(()=>{
        ...
        fetchUsers()
    }, [isAdding, SelectAccount])

  return (
    <div className={`p-5 ${isAdding || SelectAccount ? "overflow-y-clip":""}`}>
        <div className="flex flex-row justify-between items-center">
            <h1 className="text-2xl font-bold mb-5">Comptes utilisateur</h1>
            <button className="p-2 bg-primary text-white rounded-md hover:bg-[#037fc7] transition-all duration-200" onClick={()=>setIsAdding(true)}>Nouveau</button>
        </div>
        <Table Headers={Headers} Data={Users}/>
        {
            isAdding &&
            <Window Title="Ajouter un utilisateur">
                <AccountForm handleClose={()=>setIsAdding(false)}/>
            </Window>
        }
        {
            SelectAccount !== null &&
            <Window Title="Modifier">
                <AccountEditForm handleClose={()=>setSelectAccount(null)} currentUser={SelectAccount}/>
            </Window>
        }
    </div>
  )
