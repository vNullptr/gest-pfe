    ...
    const handleAssigning = async (e) =>{
        ...
        try{
            const resp = await api.patch(
                `/api/stage/${Data?.stages.find(s=>s?.statut === 0)?.id}/supervisor`,
  ...
  return (
    <>
        <div className="bg-white rounded-xl border border-gray-200 p-3 space-y-1 flex flex-col justify-evenly">
            ...
            <h2 className="text-gray-600 mb-5">Entreprise : {Data?.stages.find(s=>s?.statut === 0)?.entreprise}</h2>
            <div className="flex flex-row space-x-1">
            <button className="w-4/5 px-2 py-1 bg-primary text-white rounded-md hover:bg-[#037fc7] transition-all duration-200" onClick={()=>{setAssign(Data?.id); fetchSupervisors()}}>Assigner</button>
            <button className="w-1/5 px-2 py-1 bg-white border border-gray-300 text-gray-600 rounded-md hover:bg-gray-300 transition-all duration-200" onClick={()=>{setDetails(true); fetchDocument()}}>👁</button>
            </div>
        </div>
        ...
        {details &&
            <Window Title="Informations">
                <div className="flex flex-col space-y-3 px-5 pb-5">
                        <div>
                            <h1 className="text-xl font-bold">{Data?.stages.find(s=>s?.statut===0).entreprise}</h1>
                        </div>
                        <div>
                            <p>Nom : {Data?.nom}</p>
                        ...
                        <div>
                            <p>Debut : {Data?.stages.find(s=>s?.statut===0)?.debut}</p>
                            <p>Fin : {Data?.stages.find(s=>s?.statut===0)?.fin}</p>
                        </div>
                        <div>
                        ...
                        <div className="flex items-center justify-center">
                            <button className="p-2 bg-white border border-gray-300 text-gray-600 rounded-md hover:bg-gray-300 transition-all duration-200" onClick={()=>setDetails(false)}>Fermer</button>
                        </div>
                    </div>
            </Window>}
        {assign !== null &&
            <Window Title="Assigner un encadrant">
                <form className="flex flex-col space-y-3 px-5 pb-5" onSubmit={handleAssigning}>
                    ...
                    <div className="flex items-center justify-center space-x-2">
                        <button className="p-2 bg-white border border-gray-300 text-gray-600 rounded-md hover:bg-gray-300 transition-all duration-200" onClick={()=>setAssign(null)}>Annuler</button>
