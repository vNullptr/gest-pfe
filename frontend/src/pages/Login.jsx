...
const Login = () => {
  ...
  const [hasWrongCredit, setWrongCredit] = useState(false);
      ...
      <div className="flex flex-col items-center rounded-[5px] min-w-[400px] py-[30px] w-1/2 p-15 bg-white">
          <form className="space-y-3 flex flex-col justify-center items-center" onSubmit={(e)=>{ 
            ...
            <div className="relative">
              <input id="emailInput" className={`px-3 border border-gray-300 outline-0 rounded-md p-[3px] focus:border-primary w-90 h-11 ${ hasWrongCredit ? "border-red-500": ""}`} type="email" onChange={(e)=>{ setEmail(e.target.value); setWrongCredit(false); }} value={Email}></input>
              <label htmlFor="emailInput" className={`absolute -top-2 left-3 text-gray-400 text-[10px] bg-white px-1 ${ hasWrongCredit ? "text-red-500": ""}`}>Email</label>
            </div>
            <div className="relative">
              <input id="passInput" className={`px-3 border border-gray-300 outline-0 rounded-md focus:border-primary w-90 h-11 ${ hasWrongCredit ? "border-red-500": ""}`} type="password" onChange={(e)=>{ setPassword(e.target.value); setWrongCredit(false); }} value={Password}></input>
              <label htmlFor="emailInput" className={`absolute -top-2 left-3 text-gray-400 text-[10px] bg-white px-1 ${ hasWrongCredit ? "text-red-500": ""}`}>Mots de passe</label>
            </div>
            ...
            {hasWrongCredit && <p className='font-extralight text-[.8em] -mb-2 text-red-500'>Email ou mots de passe invalide.</p>}
